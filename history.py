...
# ----------------- main app -----------------
def main():
    st.title("Agglomerative Clustering - Stunting")
    st.write("Upload file Excel untuk analisis clustering menggunakan metode Agglomerative Clustering (tanpa dendrogram).")
    ...
    try:
        ...
        st.info(f"Menggunakan {k_final} cluster (Agglomerative)")

        ac_final = AgglomerativeClustering(n_clusters=k_final)
        labels_final = ac_final.fit_predict(X_scaled)
        df_result = df.copy()
        ...
        st.download_button("Unduh ringkasan cluster", data=excel_buf_summary, file_name="ringkasan_cluster_agglomerative.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

        excel_buf_all = to_excel_bytes(df_result)
        st.download_button("Unduh hasil lengkap", data=excel_buf_all, file_name="hasil_agglomerative_lengkap.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
